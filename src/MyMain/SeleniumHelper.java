...
public class SeleniumHelper {
    ...
    public SeleniumHelper(User user, Project project) {
    ...
    private boolean login() {
        ...
        driver.get("http://" + SetupTestPlanRunner.cloudServer.getServerHostName() + ":" + SetupTestPlanRunner.cloudServer.getPORT());

        //needs login
        try {
            _log.debug("logging in to Cloud UI");
...
//                    e.printStackTrace();
                    _log.Error("failed to navigate to correct project: " + project.getProjName());
            ...
            e.printStackTrace();
            _log.Error("failed to login - user: " + user.getUserName());
            _log.Error(e.getMessage());
            return false;
        }
        this.loggedIn = true;
    ...
    public List<TestPlan> getProjectTestPlans() {
        ...
        _log.debug("getting the project test plans");
        try {
            Thread.sleep(1500);
...
//                ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", TestPlans.get(i));
                ...
                _log.debug("chose the test plan, and get info");
                ...
                _log.debug("Finished adding the current plan to test plans from project - " + this.project.getProjName());
                _log.debug("found test plan with name" + testPlanName);
    ...
    public boolean checkIfProjectHasTestPlans() {
        ...
        try {
            ...
            _log.debug("navigate to project test plans");
            ...
            _log.debug("project - " + project.getProjId() + ", has android- " + hasAndroid + ", has IOS " + hasIOS);
            return hasAndroid && hasIOS;
        } catch (Exception e) {
//            e.printStackTrace();
            _log.Error("looks like this project " + this.project.getProjName() + " does not have test plans");
            return false;
        }
    }

    public List<TestPlan> createTestPlans() {
        _log.debug("create test plans for android + ios");
        ArrayList<TestPlan> testPlans = new ArrayList<TestPlan>();
    ...
    public TestPlan createTestPlanForOS(Enums.OS os){
        ...
        navigateToTestPlansPage();
        _log.debug("create test plan for os - " + os);
        ArrayList<String> deviceQueries = new ArrayList<>();
        ...
        _log.debug("upload the application");
        uploadApp(os);
        _log.debug("choose the devices");
        ...
        try {
            _log.debug("Created test plan, " + testPlanName +  " waiting for 5 seconds before verifying it exists");
            Thread.sleep(5000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    ...
    private void navigateToTestPlansPage(){
        _log.debug("navigate to project test plans page");
//        driver.manage().window().fullscreen();

        try {
            driver.findElement(By.xpath("//*[@st-select-row='plan']"));
            _log.debug("already in testPlansPage");
            return;
        } catch (Exception e) {
//            e.printStackTrace();
        }
...
//            e.printStackTrace();
            _log.Error("Did not navigate to test plans page");
        }
    }
    private boolean testPlanExists(String testPlan){
        _log.debug("does test plan " + testPlan + " exists?");
...
//        List<WebElement> applicationUploadButtons = driver.findElements(By.xpath("//input[@id='select-file-upload']"));
        _log.debug("uploading the application " + appPath + "for OS " + os);
        _log.debug("uploading the test application " + testAppPath + "for OS " + os);
    ...
    public void perish(){
        _log.debug("killing the selenium driver");
        driver.quit();
    }

    public boolean runTestPlanFromUI(TestPlan testPlan){
        _log.debug("run test plan from UI");
        login();
        navigateToTestPlansPage();
        List<WebElement> testPlans = driver.findElements(By.xpath("//*[@st-select-row='plan']"));
        ...
        try {
            ...
            _log.Error("could not find test plan to run from api, testPlan " + testPlan.getTestPlanName());
            return false;
        }
        testPlanElem.click();
...
//        navigateToTestPlansPage();
    ...
    private boolean verifyTestPlanStillRunning(TestPlan testPlan){
        _log.debug("verify test plan is still running");
        String runningTest = "//*[@st-select-row='test']";
        ...
        while(runningTestElem.getText().contains("Running") && (startTime + 4 * 60 * 60 * 1000) > currTime){
            try {
                _log.debug(testPlan.getTestPlanName() + " is still running.");
                Thread.sleep(15000);
                runningTestElem = driver.findElement(By.xpath(runningTest));
            } catch (InterruptedException e) {
                e.printStackTrace();
    ...
    public boolean deleteTestPlan(TestPlan testPlantoDelete){
        _log.debug("delete test plan from UI");
        login();
        navigateToTestPlansPage();
        List<WebElement> testPlans = driver.findElements(By.xpath("//*[@st-select-row='plan']"));
        WebElement testPlanElem;
        try {
            ...
            _log.Error("could not find test plan to run from api, testPlan " + testPlantoDelete.getTestPlanName());
            return false;
        }
        testPlanElem.click();
